...
class AttentionHeatmapGenerator:
	def __init__(self, screen_width=640, screen_height=360, heatmap_size=(64, 36)):
		...
		self.filter_params = {
			...
			'decay_rate': 0.8,                # 时间衰减率，用于加权距离计算
			'gaussian_std': 2.5              # 高斯分布的标准差
		}
		...
		self.normalize_param = {
            'min': [-0.5, -0.5, 0.0],   # 最小坐标
            'max': [0.5, 0.5, 0.5]      # 最大坐标
        }
	...
	def update_realtime_heatmap(self, current_time, total_duration=None, use_filtered_points=False):
		...
		if (total_duration is not None and 
			...
			if current_time >= fixed_window_start_time:
				# 使用固定窗口：叠加最后'window_seconds'秒数据 (total_duration-'window_seconds' ~ total_duration)
		...
		if use_filtered_points:
			# 使用过滤后的点
			for timestamp, gaze_x, gaze_y in self.filtered_gaze_history:
				if window_start <= timestamp <= window_end:
					# Convert to heatmap coordinates
					x_heatmap = int(gaze_x * (self.heatmap_size[1] - 1)) #在热图中的坐标大小
					y_heatmap = int(gaze_y * (self.heatmap_size[0] - 1))
			...
			# 使用所有点
			for timestamp, gaze_x, gaze_y in self.gaze_history:
				if window_start <= timestamp <= window_end:
					# Convert to heatmap coordinates
					x_heatmap = int(gaze_x * (self.heatmap_size[1] - 1))
					y_heatmap = int(gaze_y * (self.heatmap_size[0] - 1))
		...
		gaussian_std = self.filter_params['gaussian_std']
		
		# 预计算高斯核
		# kernel_radius = int(3 * gaussian_std)
		# kernel_size = 2 * kernel_radius + 1
		
		# 计算y，x坐标矩阵
		y_coords, x_coords = np.ogrid[:self.heatmap_size[0], :self.heatmap_size[1]]
		
		# 以每个当前注视点为中心添加高斯分布
		for center_x, center_y in window_points:
			# 计算每个像素到当前注视点的距离的平方
			distances_sq = (x_coords - center_x)**2 + (y_coords - center_y)**2
			
			# 应用高斯函数
			gaussian_values = np.exp(-distances_sq / (2 * gaussian_std**2))
			
			# 添加到热图（使用最大值而不是累加，避免过度饱和）
			self.realtime_heatmap = np.maximum(self.realtime_heatmap, gaussian_values)
		
		# 归一化
		max_attention_val = np.max(self.realtime_heatmap)#热图中的最高注意力值
		if max_attention_val > 0:
			self.realtime_heatmap = self.realtime_heatmap / max_attention_val
	...
	def has_sufficient_neighbors(self, point, recent_points):
		...
		# 计算高注意力值的邻居数量
		high_attention_neighbors = np.sum(neighborhood > self.filter_params['attention_threshold'] * 2)
		
		return high_attention_neighbors >= self.filter_params['min_neighbors']
	...
	def calculate_temporal_weights(self, gazePoint_num):
		"""
		计算时间衰减权重（指数衰减）
		"""
		if gazePoint_num == 0:
		...
		# 指数衰减：最近的注视点权重最高
		decay_rate = self.filter_params['decay_rate']
		weights = [decay_rate ** i for i in range(gazePoint_num -1, -1, -1)]
	...
	def visualize_results(self, timestamps, gaze_points, hand_positions, 
		...
		# 1. 全时间热图
		# 创建全时间的热图，叠加所有注视点
		full_time_heatmap = np.zeros(self.heatmap_size, dtype=np.float32)
		...
		for i in filtered_indices:
			if i < len(gaze_points):
				...
				# 高斯计算方法
				distances_sq = (x_coords - x_heatmap)**2 + (y_coords - y_heatmap)**2
				gaussian_values = np.exp(-distances_sq / (2 * gaussian_std**2))
				
				# 累加热度值
				full_time_heatmap += gaussian_values

		# 归一化（与实时热图相同的方法）
		max_attention_val = np.max(full_time_heatmap)
		if max_attention_val > 0:
			full_time_heatmap = full_time_heatmap / max_attention_val
		...
		# 显示全时间热图
		image_fulltime_heatmap = axes[0, 0].imshow(full_time_heatmap, cmap='hot', interpolation='nearest',
		...
		axes[0, 0].legend()
		plt.colorbar(image_fulltime_heatmap, ax=axes[0, 0])
		...
		if (timestamps and 
			...
			if min_length > 0:
				times_trimmed = times[:min_length] #截取之后的时间
				...
				axes[0, 1].axhline(y=avg_left_dist, color='b', linestyle='--', alpha=0.7, 
									label=f'Left Avg: {avg_left_dist:.3f}m')
				axes[0, 1].axhline(y=avg_right_dist, color='r', linestyle='--', alpha=0.7,
									label=f'Right Avg: {avg_right_dist:.3f}m')
	...
	def get_recent_gaze_points(self, current_timestamp):
		...
		"""
		if not hasattr(self, 'gaze_points_with_time') or not self.gaze_points_with_time:
			return []
		
		# 使用传入的gaze时间戳计算时间窗口
		window_seconds = self.filter_params.get('window_seconds', 2.0)
		window_start = current_timestamp - window_seconds
		
		# 清理过期数据（保留窗口内的数据）
		self.gaze_points_with_time = [
			(ts, pos) for ts, pos in self.gaze_points_with_time 
			if ts >= window_start
		]
		
		# 获取当前窗口内的数据
		recent_points = []
		for timestamp, gaze_pos in self.gaze_points_with_time:
		...
		# 	print(f"有效数据点: {len(recent_points)}/{len(self.gaze_points_with_time)}")
...
class DataCollector:
	...
	def maincb(self, Lpsm, Rpsm, camera_depthdata, cameraR, cameraFrame):
		...
		if gazepoint_position3 is not None:
			self.attention_heatmap_generator.gaze_points_with_time.append((current_time, gazepoint_position3))		
		...
		current_idx = len(self.attention_heatmap_generator.all_timestamps) - 1  #当前最新数据点的索引位置
	...
	def normalize_3d_position(self, position_3d):
		...
		"""
		# 假设工作空间范围（需要根据实际情况调整）
		workspace_min = self.attention_heatmap_generator.normalize_param['min']    # 归一化后psm最小坐标
		workspace_max = self.attention_heatmap_generator.normalize_param['max']    # 归一化后psm最大坐标
