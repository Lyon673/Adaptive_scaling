...
def visualize_report(all_labels, all_preds,
    ...
    if save_path is None:
        save_path = os.path.join(os.path.dirname(__file__), 'eval_results', 'evaluation_report.png')
...
def evaluate_model_CRF(model, dataloader, device, save_path=None):
    ...
    print("\n--- Spatial-Attention-LSTM-CRF model evaluation report ---")
    ...
    if save_path is None:
        save_path = os.path.join(os.path.dirname(__file__), 'eval_results', 'eval_report_spatial_attn_crf.png')
    visualize_report(all_labels, all_preds, title="Spatial-Attention-LSTM-CRF Evaluation Report", save_path=save_path)
...
def evaluate_segmental_metrics(
    ...
    print(f"\n{'═' * len(header)}")
    print("  Segmental Metrics – Per Sequence (Spatial-Attn-LSTM-CRF)")
    print(f"{'═' * len(header)}")
    print(header)
    print(sep)
...
def _save_segmental_table(seq_results: list, k_list: list, save_path: str) -> None:
    ...
    plt.title("Segmental Evaluation Metrics (Spatial-Attn-LSTM-CRF)", fontsize=13, fontweight='bold', pad=12)
    plt.tight_layout()
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(model_save_path):
        ...
        evaluate_segmental_metrics(
            ...
            save_path=os.path.join(dir_path, 'eval_results', 'segmental_metrics_spatial_attn.png'),
        )
