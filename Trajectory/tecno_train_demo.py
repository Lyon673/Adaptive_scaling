...
INPUT_SIZE = 16
HIDDEN_SIZE = 64
NUM_LAYERS = 6          # RF = 63 frames. Sufficient to cover anomaly duration.
NUM_STAGES = 2
NUM_CLASSES = 7
KERNEL_SIZE = 3
DROPOUT = 0.5
...
# [NEW] Data augmentation and smoothness constraints
WINDOW_SIZE = 128       # Length of random temporal crops during training
SMOOTH_WEIGHT = 0.5     # Weight for the high-frequency oscillation penalty
...
class KinematicDataset(Dataset):
    def __init__(self, states, labels, train=False, window_size=128, crop_prob=0.5):
        self.samples = []
        self.train = train
        self.window_size = window_size
        self.crop_prob = crop_prob # [NEW] 引入切片概率超参数
    ...
    def __getitem__(self, idx):
        st, lt = self.samples[idx]
        
        # [NEW] 以 crop_prob 的概率触发切片，否则保留完整序列
        if self.train and len(st) > self.window_size:
            if np.random.rand() < self.crop_prob:
                start_idx = np.random.randint(0, len(st) - self.window_size + 1)
                st = st[start_idx : start_idx + self.window_size]
                lt = lt[start_idx : start_idx + self.window_size]
...
class MultiStageLoss(nn.Module):
    def __init__(self, num_classes=7, transition_weight=0.5, smooth_weight=0.5, ignore_index=-1):
        super().__init__()
    ...
    def _smoothness_penalty(self, logits):
        """
        [NEW] Penalize high-frequency changes in prediction logic 
        by calculating MSE of log probabilities between adjacent frames.
        """
        log_probs = F.log_softmax(logits, dim=-1)
        loss = F.mse_loss(log_probs[:, 1:, :], log_probs[:, :-1, :])
        return loss
    ...
    def forward(self, stage_outputs, labels):
        ...
        for logits in stage_outputs:
            ...
            total_smooth += self._smoothness_penalty(logits)
            
        ce_avg = total_ce / M
        trans_avg = total_trans / M
        smooth_avg = total_smooth / M
        
        # [NEW] Add smoothness penalty to total loss
        total = ce_avg + self.transition_weight * trans_avg + self.smooth_weight * smooth_avg
        return total, ce_avg, trans_avg, smooth_avg
...
def train_TeCNO():
    ...
    #demo_id_list = np.delete(np.arange(148), [112])
    ...
    # [NEW] Initialize dataset with train=True and window_size for cropping
    train_dataset = KinematicDataset(train_states, train_labels, train=True, window_size=WINDOW_SIZE,crop_prob=0.3)
    ...
    # [NEW] Add smooth_weight to criterion
    criterion = MultiStageLoss(
        num_classes=NUM_CLASSES, transition_weight=0.5, smooth_weight=SMOOTH_WEIGHT, ignore_index=-1,
    ).to(device)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for sequences, labels, lengths in train_loader:
            ...
            # [NEW] Unpack smooth_loss
            loss, ce_loss, trans_loss, smooth_loss = criterion(stage_outputs, labels)
        ...
        writer.add_scalar('smooth_loss', smooth_loss.item(), epoch) # [NEW] Log smooth loss
        ...
        print(f'Epoch [{epoch+1}/{NUM_EPOCHS}], Loss: {avg_loss:.4f}, '
              ...
              f'Smooth: {smooth_loss.item():.4f}, LR: {lr_now:.6f}, '
              f'Time: {time.time()-epoch_start:.1f}s, ETA: {eta_h:02d}:{eta_m:02d}:{eta_s:02d}')
...
def evaluate_TeCNO(model):
    ...
    # [NEW] train=False ensures full sequence evaluation
    test_dataset = KinematicDataset(test_states, test_labels, train=False)
