...
NUM_LAYERS = 3        # LSTM 层数 (取消时序编码后，可以适当减少层数防过拟合)
...
# =========================================================================
# --- 新增: 双臂空间交叉注意力 (Bimanual Spatial Cross-Attention) ---
# =========================================================================
...
class BimanualSpatialAttention(nn.Module):
    def __init__(self, input_dim=8, proj_dim=16):
        ...
        # 1. 共享权重的特征投影层: W_p * X + b_p
        self.shared_proj = nn.Linear(input_dim, proj_dim)
        
        # 2. 评估主导态势的 MLP
        # 输入是 [H_L, H_R] 的拼接 (proj_dim * 2)，输出是 2 维的 logits (对应左右臂权重)
    ...
    def forward(self, x):
        ...
        x: shape (B, T, 16)
        返回: 
          tilde_X: (B, T, 16) 重加权后的特征
          alphas:  (B, T, 2) 左右臂的注意力权重，用于可视化
        """
        # 物理拆解特征
        X_L = x[:, :, :8]
        X_R = x[:, :, 8:]
        ...
        # 2. 主导权注意力计算 (Dominance Attention)
...
class SequenceLabelingLSTM_CRF(nn.Module):
    ...
    def forward(self, x, lengths, return_alphas=False):
        """
        :param return_alphas: 如果为 True，将额外返回双臂的注意力权重用于可视化
        """
        # 1. 空间自适应重加权
...
def save_model(model, filepath, additional_info=None, demo_id_list=None):
    ...
    save_dict = {
        ...
        'model_config': {
            ...
            'proj_dim': PROJ_DIM,  # [更新] 保存空间映射层维度
            'bidirectional': False,
        },
        'training_info': {
            'epochs': NUM_EPOCHS,
