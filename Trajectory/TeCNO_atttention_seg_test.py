...
# 引入TeCNO架构及其依赖
from TeCNO_attention_seg_train import TeCNO, NUM_CLASSES, BATCH_SIZE, collate_fn
...
def visualize_report(all_labels, all_preds, title: str = "Model Evaluation Report", save_path: Optional[str] = None) -> None:
    ...
    if save_path is None:
        save_path = os.path.join(os.path.dirname(__file__), 'eval_results', 'eval_report_tecno_attn.png')
...
def load_model(filepath, device='cpu'):
    """从文件加载训练好的 TeCNO (Attention) 模型"""
    checkpoint = torch.load(filepath, map_location=device)
    model_config = checkpoint['model_config']

    print("Loading Attention-TeCNO model...")
    model = TeCNO(
...
def evaluate_model(model, dataloader, device, save_path=None):
    ...
    with torch.no_grad():
        for sequences, labels, lengths in dataloader:
            ...
            # TeCNO 返回的是 (Batch, Time, Classes) 的 logits
    ...
    target_names = [CLASS_NAMES[i] if i < len(CLASS_NAMES) else f'Class {i}' for i in range(NUM_CLASSES)]
    print("\n--- Attention-TeCNO model evaluation report ---")
    ...
    if save_path is None:
        save_path = os.path.join(os.path.dirname(__file__), 'eval_results', 'eval_report_tecno_attn.png')
    visualize_report(all_labels, all_preds, title="Attention-TeCNO Evaluation Report", save_path=save_path)
...
def evaluate_segmental_metrics(
    ...
    print(f"\n{'═' * len(header)}")
    print("  Segmental Metrics – Per Sequence (Attn-TeCNO)")
    print(f"{'═' * len(header)}")
    print(header)
    print(sep)
...
def _save_segmental_table(seq_results: list, k_list: list, save_path: str) -> None:
    ...
    plt.title("Segmental Evaluation Metrics (Attn-TeCNO)", fontsize=13, fontweight='bold', pad=12)
    plt.tight_layout()
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
...
def evaluate_model_real_time_simulation(model, test_dataset, device, save_path=None):
    ...
    if save_path is None:
        save_path = os.path.join(os.path.dirname(__file__), 'eval_results', 'eval_report_attn_tecno_realtime.png')
    visualize_report(all_labels, all_preds, title="Attention-TeCNO Real-Time Simulation Report", save_path=save_path)
...
if __name__ == "__main__":
    ...
    # 默认读取最新训练出的 Attention-TeCNO 模型
    model_save_path = os.path.join(dir_path, "TeCNO_model", "tecno_attention_sequence_model.pth")
    ...
    if not os.path.exists(model_save_path):
        ...
        evaluate_segmental_metrics(
            ...
            save_path=os.path.join(dir_path, 'eval_results', 'segmental_metrics_attn_tecno.png'),
        )
