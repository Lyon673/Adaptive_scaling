...
class DataCollector:
	...
	def maincb(self,Lpsm,Rpsm,camera_depthdata, cameraR, cameraFrame):
		...
		scale = self.calculate_scale(psm_position, psm_velocity, ipaL_data, ipaR_data, distance_psms, is_valid)	
		try:  
	...
	def calculate_scale(self, psm_position, psm_velocity, IPAL, IPAR, distance_psms, is_valid):
		...
		# 计算热图加权距离
		weighted_dist_L = self.attention_heatmap_generator.calculate_weighted_distance(normalized_Lpsm)
		weighted_dist_R = self.attention_heatmap_generator.calculate_weighted_distance(normalized_Rpsm)
